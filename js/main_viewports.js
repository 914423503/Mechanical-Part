//Constant
var OBJ_COUNT = [12, 8, 19, 28,105,3];
...
var PIP_BG = 0xcaec67;
//
var lastSELECTED,clickSELECTED;
var OPACITY = 0.5;
var NON_OPACITY = 1;
...
function init() {
	...
	for (var i = 0; i < OBJ_COUNT[currentObj]; i++) {
		...
		var qb = 'obj/' + 'obj' + currentObj + "/" + num +".obj";
		loader.load(qb, '', makeHandler(i), onProgress, onError);
	}
	//
	for (var i = 0; i < OBJ_COUNT[currentObj]; i++) {
		var num = i
		...
		var qb = 'obj/' + 'obj' + currentObj + "/" + num +".obj";
		loader.load(qb, '', makeHandler_pip(i), onProgress, onError);
	}
	//renderer
	if (webglAvailable()) {
...
function addEventListeners() {
	var listener = new DocumentMouseListener();
	listener.addModeListeners();
}
...
function DocumentMouseListener(){
	var hasMoveMode = false;
	var mousemove,mousedown,mouseup,click;

	this.click = function(event){
		event.preventDefault();
		if(clickSELECTED && lastSELECTED && clickSELECTED.id==lastSELECTED.id){
			// var moonDiv = document.getElementById('dialog');
	...
	this.addModeListeners = function(){
		 renderer.domElement.addEventListener('mousemove', this.mousemove, false);
		 renderer.domElement.addEventListener('mousedown', this.mousedown, true);
		 renderer.domElement.addEventListener('mouseup', this.mouseup, false);
		 renderer.domElement.addEventListener('click', this.click, false);
